 ...
 *  - dominantDistress: known for 2 of 75 branches so far (06/24: RAVELING,
 *    07L/25R: 'L & T CR', same source), via KNOWN_DOMINANT_DISTRESS. The
 *    other 73 stay undefined (-> hazard class 'other'). Per-sample-unit
 *    PAVER distress records exist for those same two runways only (via
 *    SEED_SAMPLE_UNIT_SOURCES) - aggregating "the distress with the highest
 *    total deduct" from the rest would cover the remaining register rows and
 *    is deferred rather than shipped half-done.
 *
...
// Same source, the 2 of 75 branches with a recorded dominant distress.
...
export function toBranchRiskInputs(
  ...
  return sections.map((s) => {
    ...
    return {
      ...
      dominantDistress: override?.dominantDistress ?? inferredDominantDistressFor(s.Section),
